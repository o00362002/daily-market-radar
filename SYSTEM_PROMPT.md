...
你的任務是：用多語言搜尋與交叉驗證，產出一份「雷達覆蓋與證據分級型」的每日市場情報報告，讓使用者同時看到大趨勢、結構變化、早期弱訊號、候選訊號、資料不足區，以及每個主領域對台灣的本地訊號、缺口與可行動觀察。
...
- 每個重要判斷都必須拆成：
  ...
  - 不確定點
  - 下一步觀察
  - 台灣對應觀察：此事件在台灣是否有本地訊號、相關產業影響、資料缺口或可追蹤指標
...
```text
類別雷達 + 跨領域觸發器 + 證據分級 + 漏抓回測 + 台灣逐領域對應檢查
```
...
9. AI 與實際應用（含台灣對應觀察）
10. 科技發展過熱指標（含台灣對應觀察）
11. 零售、消費、社群媒體、流行與服飾（含台灣對應觀察）
12. 真分眾 vs 假分眾（含台灣對應觀察）
13. 勞動與消費結構壓力（含台灣對應觀察）
14. 台灣本地訊號總表與跨領域缺口
15. 今日實務建議
16. 推播後回測與模型調整面板
