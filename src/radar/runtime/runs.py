...
from radar.domain.models import CoverageCell, ReportItem, RunResult, stable_id
...
def _empty_structural_indicators(indicator_ids: list[str]) -> list[dict[str, Any]]:
    return [
        {
            ...
            "counter_signal_ids": [],
            "missing_data": ["fixture runtime does not evaluate live structural evidence"],
            "one_sentence_read": "Insufficient live evidence in fixture mode.",
            "next_verification": ["run live ingestion and indicator-specific data checks"],
        }
...
def run_daily_fixture(
    ...
    run_id = stable_id("run", [str(repo_root), date, profile_name, ",".join(degradation_reasons)])
    ...
    report: dict[str, Any] = {
        ...
        "source_audit": {
            ...
            "fresh_rss_checked": freshrss_available,
            "external_discovery_checked": external_discovery_available,
            "remaining_gap": ["live network ingestion is not active in fixture mode"],
        },
        "rejection_counters": {
            "duplicate_rejection_count": 0,
            "field_overlap_rejection_count": 0,
            "niche_low_novelty_rejection_count": 0,
            "candidate_retry_paths_used": [] if external_discovery_available else ["external discovery unavailable"],
            "taiwan_qualified_item_count_after_audit": direct_taiwan_count,
            "taiwan_direct_sources_checked": ["twse"] if direct_taiwan_count else [],
        },
        "retail_matrix": _empty_matrix(contract.retail_matrix_keys, "fixture mode: matrix not fully evaluated"),
        "crypto_matrix": _empty_matrix(contract.crypto_matrix_keys, "fixture mode: quantitative data not evaluated"),
        "structural_indicators": _empty_structural_indicators(contract.structural_indicator_ids),
        "backtest": {
            "status": "partial",
            "findings": ["fixture run validates structure, not live-news completeness"],
            "next_adjustments": ["enable live adapters", "persist run records", "expand Taiwan and niche sources"],
        },
    ...
    return RunResult(
        ...
        domains_seen=sorted({doc.primary_domain for doc in documents}, key=contract.report_domains.index),
