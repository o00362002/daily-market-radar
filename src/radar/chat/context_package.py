...
human-produced report against that context: the context hash, package version,
report contract, date, profile, run_id, allowed event/document/source ids, evidence
urls, numeric facts, domains, matrix keys, indicator ids, Taiwan direct-evidence
rules and Major/Potential overlap.
"""
...
def build_chat_package(report: RadarReportV2, events: list[Event], contract: RuntimeContract) -> ChatPackage:
    ...
    # Content files excluding the manifest (which carries the hash).
...
You are a semantic assistant, not the judge of facts. Using only the bounded context in this package:

- Do NOT invent URLs, event ids, document ids, source ids or numeric facts.
- Keep every evidence link exactly as provided; Taiwan direct evidence must come from the listed Taiwan sources only.
- Respect the report schema (expected-output.schema.json) and the runtime contract exactly.
- Major and Potential lanes must not contain the same event.
- Return a single RadarReportV2 JSON that validates against report-schema.json, reusing run_id, date and profile.

The import step re-validates everything deterministically and rejects any drift.
"""
