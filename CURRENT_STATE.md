...
AI analysis contract：AIAnalysisV1＋config/ai_analysis.json。
...
Validated boundary：RadarReportV2、AIAnalysisV1 與 WebArtifactV1 為 strict typed contracts；AIAnalysisV1 只引用已驗證 report/event ids，不能改寫 RadarReportV2 或 deterministic indicator values。
...
固定輸出：Product/Social Competitor Watch、Retail matrix、Crypto matrix、勞動與消費壓力 indicator-only row、三個 Structural Trend Indicators、coverage gaps、rejection counters、backtest、獨立 AI 解讀／翻譯／未來趨勢／連動指標頁。
```
...
AI analysis：獨立 AIAnalysisV1 以 RadarReportV2 為唯讀輸入，產生翻譯、跨事件統整、條件式未來趨勢與六個連動指標；指標公式 deterministic，AI 僅能解讀；provider/model/time/run/hash/prompt/schema/fallback 全部留痕。無 API key 或 invalid output 會安全降級為 deterministic analysis。
...
Web projection：validated report → typed immutable artifacts；同日 re-run 依 durable write order 取最後寫入，run_id 不參與時間排序。Astro dashboard 為繁中、zero-JS、Pages-compatible，首頁區分精選與完整候選池並保留原文查核；/analysis 讀取獨立 AIAnalysisV1 artifact。
Daily automation：daily-intelligence 以 --mode live 執行 direct RSS/Atom＋optional FreshRSS；cron '0 23 * * *' UTC＝07:00 台灣；durable radar-state branch 保存 compressed+checksum SQLite；Pages 只部署 validated artifact，並將 JSON artifacts 一併發佈於 /data/。ai-analysis 在 daily-intelligence/import-chat 成功後再產生解讀層並重新部署。
...
尚未完成：web/API/social、fixed queries/coverage retry 與 external discovery 接線；逐來源 health 及 fresh/backfill 分層；真實 AI key 線上驗證；typed competitor payload/history；獨立 AI analysis 長期 repository/history；read-only「問雷達」MCP。
無 AI key 時不做外部標題機器翻譯，RadarReportV2 仍保留原文並明確顯示 deterministic 模式；AIAnalysisV1 顯示 deterministic fallback。
...
config/ai_analysis.json = AI 解讀與連動指標設定
