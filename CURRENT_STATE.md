...
尚未完成：多 adapter 收集 stage 併入 run-daily（health 記錄／registry 選路）、real API-key 線上驗證、daily scheduler、durable radar-state branch、GitHub Pages 部署 workflow、web/workflow CI、secrets contract（.env.example/redaction）。
