...
<!-- 頭部只保留現在成立的事實；歷史移至 reports/ 或 archive/。可由 canonical config 推導的數量不得手抄。 -->

## 現況摘要

```text
掛載：brain-core child mount；治理 owner 與本地 consumer mapping 見 brain.manifest.yaml。
...
Report-domain membership、aliases 與 profile floors：只由 config/runtime_contract.json 擁有；labor 預設 indicator-only；競品採 cross-domain projection，不新增領域或重複計數。
Structural indicators：清單與證據規格由 config/runtime_contract.json 與 configs/structural_trend_indicators.yml 擁有；不得在 State 手抄現行數量。
Indicator hierarchy：structural indicators 是長期主層；auxiliary signal panel 是輔助訊號，不得互相取代，成員由其 canonical config 擁有。
...
AI analysis：AIAnalysisV1 產生翻譯、跨事件統整與條件式未來趨勢；/analysis 的閱讀順序由現行 web/read-model contract 決定。AI 不得改寫 deterministic 指標；provider/model/time/run/hash/prompt/schema/fallback 全留痕。
...
```text
AGENTS.md = 第一入口
README.md = 專案能力與核心指標快速索引
CURRENT_STATE.md = 現況
CURRENT_DECISIONS.md = 已接受決策
docs/structural-indicators.md = 核心結構指標的人類可讀權威入口
