...
Web projection：validated RadarReportV2 → typed web read-models（WebManifestV1 等，schema→TS 生成，drift-tested）→ content-addressed immutable artifacts（atomic stage-then-replace、unchanged skip、incremental、失敗不留半套）via `radar export-web`；Astro static dashboard（zero-JS、native CSS、繁中、Pages base 由 env、事實/推論/反證/不確定/缺口與 mode/status/fixture 皆 text+symbol 標示、history 分頁、SVG sparkline、空窗=insufficient）build 通過（14 頁、0KB JS）。
Daily automation（PR F）：六個 workflows（runtime-check/web-check/daily-intelligence/prepare-chat/import-chat/pages-deploy）＋保留 mount-check；daily-intelligence cron '0 23 * * *' UTC＝07:00 台灣、09:00 前部署，RADAR_EVALUATION_MODE 預設 auto；durable radar-state branch（compressed+checksum SQLite、last-good backup、atomic、concurrency lock、corruption rollback，radar state pack/verify/restore）；Pages 只部署 validated artifact（failed 不部署、fixture 僅 preview、base 由 env）；secrets contract（.env.example、docs/secrets.md、redaction 不記錄任何憑證，全部 optional，無 secret 仍可 deterministic＋建站＋部署）。
尚未完成：多 adapter 收集 stage 併入 run-daily（health 記錄／registry 選路）、以真實 API key 線上驗證 AI evaluation；GitHub Actions／Pages 需 owner 在 UI 開啟（Pages source=Actions、Actions 讀寫權、optional secrets）後方會真正執行部署。
