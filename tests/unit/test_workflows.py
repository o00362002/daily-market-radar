...
class WorkflowContractTests(unittest.TestCase):
    ...
    def test_weekly_pipeline_has_utc_cron_concurrency_and_production_gates(self) -> None:
        ...
        self.assertIn("Capture Taiwan report date", text)
        self.assertIn('id: report_date', text)
        self.assertEqual(text.count('DATE="${{ steps.report_date.outputs.value }}"'), 3)
        self.assertNotIn("DATE=$(TZ=Asia/Taipei date +%F)", text)
